...
from utils.ollama_client import OllamaModelWrapper
...
def research_citations_via_api(
    ...
    if use_deep_research:
        ...
        try:
            ...
            # If a translated English topic is available, prepend English queries
            # (Crossref, OpenAlex, Semantic Scholar are English-first databases)
            topic_en = os.environ.get('SEARCH_TOPIC_EN', '').strip()
            if topic_en and topic_en.lower() != topic.lower():
                en_queries = [
                    topic_en,
                    f"{topic_en} history",
                    f"{topic_en} Islamic studies",
                    f"{topic_en} research",
                    f"{topic_en} analysis",
                ]
                research_topics = en_queries + research_topics
                if verbose:
                    safe_print(f"   Added {len(en_queries)} English-translated queries: {topic_en[:60]}...")
    ...
    # === ENGLISH QUERY INJECTION ===
    # Crossref, OpenAlex, Semantic Scholar are English-first databases.
    # If topic is in another language, prepend English-translated queries
    # so the databases actually return results.
    _topic_en = os.environ.get('SEARCH_TOPIC_EN', '').strip()
    if not _topic_en and topic:
        # Try to translate on-the-fly if env var wasn't set
        try:
            from deep_translator import GoogleTranslator
            _topic_en = GoogleTranslator(source='auto', target='en').translate(topic[:500])
        except Exception:
            _topic_en = ''

    if _topic_en and _topic_en.lower() != (topic or '').lower():
        en_queries = [
            _topic_en,
            f"{_topic_en} history",
            f"{_topic_en} Islamic studies",
            f"{_topic_en} research",
            f"{_topic_en} culture heritage",
            f"Hanafi jurisprudence Iraq",
            f"Islamic scholarship Baghdad",
        ]
        # Prepend so they run first (most likely to succeed)
        research_topics = en_queries + [t for t in research_topics if t not in en_queries]
        if verbose:
            safe_print(f"   🌍 English queries prepended ({len(en_queries)}): {_topic_en[:60]}...")
    # === END ENGLISH QUERY INJECTION ===
    ...
    enable_semantic_scholar = os.environ.get('ENABLE_SEMANTIC_SCHOLAR', '1') == '1'
    enable_crossref = os.environ.get('ENABLE_CROSSREF', '1') == '1'
    enable_openalex = os.environ.get('ENABLE_OPENALEX', '1') == '1'
    enable_web = os.environ.get('ENABLE_WEB', '1') == '1'

    # Allow UI to override the minimum citation target
    _min_cit_env = os.environ.get('MIN_CITATIONS', '')
    if _min_cit_env.isdigit():
        target_minimum = int(_min_cit_env)
    ...
    researcher = CitationResearcher(
        ...
        enable_gemini_grounded=enable_web,  # Web araması
        enable_smart_routing=True,     
        enable_llm_fallback=False,     
        use_serper=enable_web,               
        verbose=verbose,
