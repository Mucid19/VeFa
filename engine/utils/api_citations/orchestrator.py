...
class CitationResearcher:
    ...
    def __init__(
        ...
        self.enable_wikipedia = os.getenv('ENABLE_WIKIPEDIA', '1') == '1'
        self.enable_duckduckgo = os.getenv('ENABLE_DUCKDUCKGO', '1') == '1'
    ...
    def _search_api(self, api_name: str, topic: str) -> Tuple[Optional[Dict[str, Any]], str]:
        ...
        try:
            ...
            elif api_name == 'wikipedia' and self.enable_wikipedia:
                logger.debug(f"  → Calling Wikipedia API...")
                metadata = self.wikipedia.search_paper(topic)
                if metadata:
                    logger.info(f"  ✓ Wikipedia found: {metadata.get('title', 'Unknown')[:80]}")
                    return (metadata, "Wikipedia")
                else:
                    logger.debug(f"  ✗ Wikipedia returned no results")
            elif api_name == 'duckduckgo' and self.enable_duckduckgo:
                logger.debug(f"  → Calling DuckDuckGo Search...")
                metadata = self.duckduckgo.search_paper(topic)
                if metadata:
                    logger.info(f"  ✓ DuckDuckGo found: {metadata.get('title', 'Unknown')[:80]}")
                    return (metadata, "DuckDuckGo")
                else:
                    logger.debug(f"  ✗ DuckDuckGo returned no results")
