...
def setup_output_folders(output_dir: Path, clean_fresh: bool = False) -> Dict[str, Path]:
    """
    ...
    if clean_fresh:
        ...
        if checkpoint.exists():
            try:
                ...
                pass
        for k in ['papers', 'drafts', 'exports']:
            if folders[k].exists():
                shutil.rmtree(folders[k], ignore_errors=True)
        if folders['research'].exists():
            for item in folders['research'].iterdir():
                if item.is_file():
                    try:
...
def get_word_count_targets(academic_level: str) -> dict:
    ...
    result = targets.get(academic_level, targets['master']).copy()

    # --- UI overrides via environment variables ---
    import os as _os
    _min_cit = _os.environ.get('MIN_CITATIONS', '')
    if _min_cit.isdigit():
        result['min_citations'] = int(_min_cit)
        result['deep_research_min_sources'] = max(int(_min_cit) * 2, 5)

    _length = _os.environ.get('RESEARCH_LENGTH', '')  # 'short' | 'medium' | 'long'
    if _length == 'short':
        for k, v in list(result.items()):
            if isinstance(v, str) and '-' in v:
                parts = v.replace(',', '').split('-')
                if len(parts) == 2 and all(p.isdigit() for p in parts):
                    result[k] = f"{int(parts[0])//2:,}-{int(parts[1])//2:,}"
    elif _length == 'long':
        for k, v in list(result.items()):
            if isinstance(v, str) and '-' in v:
                parts = v.replace(',', '').split('-')
                if len(parts) == 2 and all(p.isdigit() for p in parts):
                    result[k] = f"{int(int(parts[0])*1.5):,}-{int(int(parts[1])*1.5):,}"
...
def generate_draft(
    ...
    try:
        ...
        folders = setup_output_folders(output_dir, clean_fresh=(resume_from is None))
