...
def run_compile_and_export(ctx: DraftContext) -> Tuple[Path, Path]:
    ...
    is_turkish = str(ctx.language or 'tr').lower().startswith('tr') or str(ctx.language).lower() == 'turkish'
    abstract_heading = "## Özet" if is_turkish else "## Abstract"
    intro_heading = "# 1. Giriş" if is_turkish else "# 1. Introduction"
    body_heading = "# 2. Ana Bölüm" if is_turkish else "# 2. Main Body"
    conc_heading = "# 3. Sonuç" if is_turkish else "# 3. Conclusion"

    if appendix_clean.strip():
        app_title = "# 4. Ekler" if is_turkish else "# 4. Appendices"
        ref_title = "# 5. Kaynakça" if is_turkish else "# 5. References"
        appendices_block = f"{app_title}\n{appendix_clean}\n\n\\newpage\n\n{ref_title}"
    else:
        ref_title = "# 4. Kaynakça" if is_turkish else "# 4. References"
        appendices_block = ref_title
...
"""
    ...
    if getattr(ctx, 'skip_pdf', False):
        logger.info("PDF export skipped by configuration")
    else:
        pdf_success = export_pdf(md_file=final_md_path, output_pdf=pdf_path, engine='auto')
        if not pdf_success:
            logger.warning("PDF export failed - Continuing to DOCX export.")
        elif not pdf_path.exists():
            logger.warning(f"PDF export failed - file not created: {pdf_path}")
